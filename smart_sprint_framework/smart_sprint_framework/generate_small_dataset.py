...
def generate_small_developers_csv():
    # First, let's load the tickets to calculate workload
    try:
        tickets_df = pd.read_csv('sprint_documents_small.csv')
        # Calculate workload for each developer
        developer_workload = {}
        for _, ticket in tickets_df.iterrows():
            if pd.notna(ticket['assigned_to']):
                dev_id = int(ticket['assigned_to'])
                if dev_id not in developer_workload:
                    developer_workload[dev_id] = 0
                developer_workload[dev_id] += ticket['estimated_hours']
    except:
        developer_workload = {}
    
    developers_data = [
        {'id': 1, 'name': 'John Smith', 'skills': '["python", "react", "mysql", "docker"]', 'availability': 40, 'current_workload': developer_workload.get(1, 0), 'experience_level': 4},
        {'id': 2, 'name': 'Sarah Johnson', 'skills': '["redis", "git", "mysql", "jenkins"]', 'availability': 40, 'current_workload': developer_workload.get(2, 0), 'experience_level': 1},
        {'id': 3, 'name': 'Mike Williams', 'skills': '["java", "spring", "postgresql", "docker"]', 'availability': 40, 'current_workload': developer_workload.get(3, 0), 'experience_level': 3},
        {'id': 4, 'name': 'Emma Davis', 'skills': '["javascript", "react", "node.js", "mongodb"]', 'availability': 40, 'current_workload': developer_workload.get(4, 0), 'experience_level': 4},
        {'id': 5, 'name': 'Alex Miller', 'skills': '["python", "django", "postgresql", "redis"]', 'availability': 40, 'current_workload': developer_workload.get(5, 0), 'experience_level': 5},
        {'id': 6, 'name': 'Lisa Brown', 'skills': '["java", "spring", "mysql", "kubernetes"]', 'availability': 40, 'current_workload': developer_workload.get(6, 0), 'experience_level': 3},
        {'id': 7, 'name': 'David Wilson', 'skills': '["typescript", "angular", "node.js", "mongodb"]', 'availability': 40, 'current_workload': developer_workload.get(7, 0), 'experience_level': 4},
        {'id': 8, 'name': 'Olivia Taylor', 'skills': '["python", "flask", "sqlite", "docker"]', 'availability': 40, 'current_workload': developer_workload.get(8, 0), 'experience_level': 2},
        {'id': 9, 'name': 'James Garcia', 'skills': '["c#", ".net", "sql server", "azure"]', 'availability': 40, 'current_workload': developer_workload.get(9, 0), 'experience_level': 4},
        {'id': 10, 'name': 'Sophia Rodriguez', 'skills': '["php", "laravel", "mysql", "aws"]', 'availability': 40, 'current_workload': developer_workload.get(10, 0), 'experience_level': 3}
    ]
...
def generate_small_sprint_documents_csv():
    ...
    for i in range(1, 36):  # IDs from 1 to 35
        ...
        # Assign a developer if the ticket is in progress or completed
        assigned_to = None
...
def main():
    ...
    # Then generate developers with correct workload
    generate_small_developers_csv()
    # Finally generate performance data

...
function App() {
        ...
        const verifyToken = async () => {
            if (token) {
                try {
                    ...
                    setToken(null);
                    setIsAuthenticated(false);
                    setAuthError('Failed to connect to the server. Please make sure the backend is running on port 5001.');
                } finally {
                    setLoading(false);
                }
            } else {
    ...
    const login = async (username, password) => {
        try {
            ...
            setAuthError(error.message || 'Login failed. Please make sure the backend is running on port 5001.');
            return false;
        }
    };
    ...
    const handleCreateTicket = async (e) => {
        e.preventDefault();
        try {
            const response = await apiRequest(`${API_BASE_URL}/api/tickets`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const renderNavigation = () => (
        <div className="navigation-panel">
            ...
            <div className="nav-buttons">
                ...
                <button className={`nav-btn ${activeView === 'tickets' ? 'active' : ''}`} onClick={() => setActiveView('tickets')}>1. View all tickets</button>
                <button className={`nav-btn ${activeView === 'developers' ? 'active' : ''}`} onClick={() => setActiveView('developers')}>2. View all developers</button>
                <button className={`nav-btn ${activeView === 'create' ? 'active' : ''}`} onClick={() => setActiveView('create')}>3. Process new feature story</button>
                <button className={`nav-btn ${activeView === 'assign' ? 'active' : ''}`} onClick={() => { setActiveView('assign'); setSelectedTicket(null); setRecommendations([]); setTimeline(null); }}>4. Get recommendations, estimate timeline, and assign developer</button>
                <button className={`nav-btn ${activeView === 'complete' ? 'active' : ''}`} onClick={() => { setActiveView('complete'); setSelectedTicket(null); }}>5. Complete ticket</button>
                <button className={`nav-btn ${activeView === 'status' ? 'active' : ''}`} onClick={() => setActiveView('status')}>6. View system status</button>
                <button className={`nav-btn ${activeView === 'performance' ? 'active' : ''}`} onClick={() => { setActiveView('performance'); setSelectedDeveloper(null); setDeveloperPerformance(null); }}>7. View developer performance</button>
                <button className={`nav-btn ${activeView === 'document' ? 'active' : ''}`} onClick={() => setActiveView('document')}>8. Process document</button>
                <button className={`nav-btn ${activeView === 'sprint' ? 'active' : ''}`} onClick={() => setActiveView('sprint')}>9. Process sprint document with user stories</button>
                <button className={`nav-btn ${activeView === 'save' ? 'active' : ''}`} onClick={() => setActiveView('save')}>10. Save data manually</button>
                <button className="nav-btn exit-btn" onClick={() => { if (window.confirm('Are you sure you want to exit?')) { alert('Thank you for using Smart Sprint!'); window.close(); } }}>11. Exit</button>
            </div>
        </div>
    );
                        ...
                        {tickets.map(ticket => {
                            ...
                            return (
                                <tr key={ticket.id}>
                                    <td>{ticket.id}</td>
                                    <td>{ticket.title}</td>
                                    <td><span className="status-badge" style={{ backgroundColor: getStatusColor(ticket.status) }}>{ticket.status.replace('_', ' ')}</span></td>
                                    <td><span className="priority-badge" style={{ backgroundColor: getPriorityColor(ticket.priority) }}>{ticket.priority}</span></td>
                                    ...
                                    <td>
                                        <div className="action-buttons">
                                            <button className="btn btn-sm" onClick={() => { setSelectedTicket(ticket); setActiveView('viewTicket'); }}>View</button>
                                            {ticket.status === 'backlog' && <button className="btn btn-sm btn-primary" onClick={() => { setSelectedTicket(ticket); setActiveView('assign'); }}>Assign</button>}
                                            {ticket.status === 'in_progress' && <button className="btn btn-sm btn-success" onClick={() => { setSelectedTicket(ticket); setActiveView('complete'); }}>Complete</button>}
                                            <button className="btn btn-sm" onClick={() => handleExportToJira(ticket.id)} disabled={ticket.jira_id}>{ticket.jira_id ? 'Exported' : 'Export to Jira'}</button>
                                        </div>
                                    </td>
                                </tr>
                            );
        ...
        if (!selectedTicket) {
            return (
                <div className="view-ticket-view">
                    ...
                    <button className="btn btn-primary" onClick={() => setActiveView('tickets')}>Back to Tickets</button>
                </div>
            );
        }
                ...
                <div className="ticket-details">
                    <div className="ticket-header">
                        ...
                        <div className="ticket-meta">
                            ...
                            <span className="status-badge" style={{ backgroundColor: getStatusColor(selectedTicket.status) }}>{selectedTicket.status.replace('_', ' ')}</span>
                            <span className="priority-badge" style={{ backgroundColor: getPriorityColor(selectedTicket.priority) }}>{selectedTicket.priority}</span>
                        </div>
                    </div>
                    ...
                    <div className="action-buttons">
                        <button className="btn btn-secondary" onClick={() => setActiveView('tickets')}>Back to Tickets</button>
                        <button className="btn btn-primary" onClick={() => setActiveView('dashboard')}>Back to Dashboard</button>
                        {selectedTicket.status === 'backlog' && <button className="btn btn-primary" onClick={() => setActiveView('assign')}>Assign Ticket</button>}
                        {selectedTicket.status === 'in_progress' && <button className="btn btn-success" onClick={() => setActiveView('complete')}>Complete Ticket</button>}
                    </div>
                </div>
            </div>
        );
                ...
                {developers.map(developer => (
                    <div key={developer.id} className="developer-card">
                        <div className="developer-header">
                            <div className="developer-avatar">{developer.name.split(' ').map(n => n[0]).join('')}</div>
                            <div>
                        ...
                        <div className="developer-stats">
                            ...
                            <div className="stat">
                                ...
                                <span className="stat-value">{(developer.current_workload / developer.availability * 100).toFixed(1)}%</span>
                            </div>
                        </div>
                        ...
                        <button className="btn btn-primary" onClick={() => { setSelectedDeveloper(developer); setDeveloperPerformance(null); setActiveView('performance'); }}>View Performance</button>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="create-ticket-view">
            ...
            <form className="create-ticket-form" onSubmit={handleCreateTicket}>
                <div className="form-group">
                    ...
                    <input type="text" id="title" value={newTicket.title} onChange={(e) => setNewTicket({ ...newTicket, title: e.target.value })} required />
                </div>
                <div className="form-group">
                    <label htmlFor="description">Description</label>
                    <textarea id="description" value={newTicket.description} onChange={(e) => setNewTicket({ ...newTicket, description: e.target.value })} rows="4" required></textarea>
                </div>
                <div className="form-row">
                    <div className="form-group">
                        <label htmlFor="priority">Priority</label>
                        <select id="priority" value={newTicket.priority} onChange={(e) => setNewTicket({ ...newTicket, priority: e.target.value })}>
                    ...
                    <div className="form-group">
                        ...
                        <input type="number" id="estimated_hours" value={newTicket.estimated_hours} onChange={(e) => setNewTicket({ ...newTicket, estimated_hours: parseInt(e.target.value) })} min="1" required />
                    </div>
                </div>
            ...
            {!selectedTicket ? (
                    ...
                    <div className="ticket-list">
                        {tickets.filter(t => t.status === 'backlog').map(ticket => (
                            <div key={ticket.id} className="ticket-select-item" onClick={() => setSelectedTicket(ticket)}>
                                ...
                                <span className="ticket-priority" style={{ backgroundColor: getPriorityColor(ticket.priority) }}>{ticket.priority}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="assign-ticket-content">
                    ...
                    <div className="recommendation-section">
                        <div className="action-buttons">
                            <button className="btn btn-primary" onClick={() => handleGetRecommendations(selectedTicket.id)} disabled={recommendations.length > 0}>Get Recommendations</button>
                            <button className="btn btn-secondary" onClick={() => { setSelectedTicket(null); setRecommendations([]); setTimeline(null); }}>Back</button>
                        </div>
                                    ...
                                    {recommendations.map((rec, index) => (
                                        <div key={index} className="developer-option">
                                            <div className="developer-info">
                                                <div className="developer-avatar">{rec.developer_name.split(' ').map(n => n[0]).join('')}</div>
                                                <div>
                                                    <h4>{rec.developer_name}</h4>
                                                    <div className="developer-match"><span>Match Score: {rec.match_score.toFixed(2)}</span></div>
                                                </div>
                                            </div>
                                            <button className="btn btn-success" onClick={() => { handleAssignTicket(selectedTicket.id, rec.developer_id); setSelectedTicket(null); setRecommendations([]); setTimeline(null); }}>Assign</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <div className="complete-ticket-content">
                    ...
                    <div className="completion-form">
                        ...
                        <div className="form-group">
                            ...
                            <input type="number" id="completion_time" value={completeTicketData.completion_time} onChange={(e) => setCompleteTicketData({ ...completeTicketData, completion_time: parseFloat(e.target.value) })} min="0.1" step="0.1" required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="revisions">Number of Revisions</label>
                            <input type="number" id="revisions" value={completeTicketData.revisions} onChange={(e) => setCompleteTicketData({ ...completeTicketData, revisions: parseInt(e.target.value) })} min="0" required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="sentiment_score">Sentiment Score (0-1)</label>
                            <input type="number" id="sentiment_score" value={completeTicketData.sentiment_score} onChange={(e) => setCompleteTicketData({ ...completeTicketData, sentiment_score: parseFloat(e.target.value) })} min="0" max="1" step="0.01" required />
                        </div>
                        <div className="form-actions">
                            <button className="btn btn-success" onClick={() => handleCompleteTicket(selectedTicket.id)}>Complete Ticket</button>
    ...
    const renderSystemStatusView = () => (
        <div className="system-status-view">
            ...
            <div className="status-cards">
                <div className="status-card"><h3>Total Tickets</h3><div className="status-value">{systemStatus.total_tickets}</div></div>
                <div className="status-card"><h3>Completed Tickets</h3><div className="status-value">{systemStatus.completed_tickets}</div></div>
                <div className="status-card"><h3>In Progress Tickets</h3><div className="status-value">{systemStatus.in_progress_tickets}</div></div>
                <div className="status-card"><h3>Backlog Tickets</h3><div className="status-value">{systemStatus.backlog_tickets}</div></div>
                <div className="status-card"><h3>Total Workload</h3><div className="status-value">{systemStatus.total_workload} hours</div></div>
                <div className="status-card"><h3>Total Availability</h3><div className="status-value">{systemStatus.total_availability} hours</div></div>
                <div className="status-card"><h3>Utilization Rate</h3><div className="status-value">{systemStatus.utilization_rate?.toFixed(1)}%</div></div>
            </div>
            <div className="action-buttons">
                <button className="btn btn-primary" onClick={handleOptimizeWorkload}>Optimize Workload</button>
            ...
            {!selectedDeveloper ? (
                <div className="developer-selection">
                    ...
                    <div className="developer-list">
                        {developers.map(developer => (
                            <div key={developer.id} className="developer-select-item" onClick={() => { setSelectedDeveloper(developer); fetchDeveloperPerformance(developer.id); }}>
                                <div className="developer-avatar">{developer.name.split(' ').map(n => n[0]).join('')}</div>
                                <div>
                ...
                <div className="performance-details">
                    <div className="developer-header">
                        <div className="developer-avatar">{selectedDeveloper.name.split(' ').map(n => n[0]).join('')}</div>
                        <div>
                    ...
                    <div className="performance-stats">
                        <div className="performance-stat"><h4>Availability</h4><p>{selectedDeveloper.availability} hours</p></div>
                        <div className="performance-stat"><h4>Current Workload</h4><p>{selectedDeveloper.current_workload} hours</p></div>
                        <div className="performance-stat"><h4>Utilization</h4><p>{(selectedDeveloper.current_workload / selectedDeveloper.availability * 100).toFixed(1)}%</p></div>
                        <div className="performance-stat"><h4>Skills</h4><div className="skills-list">{selectedDeveloper.skills.map((skill, index) => (<span key={index} className="skill-tag">{skill}</span>))}</div></div>
                    </div>
                    {developerPerformance ? (
                        <div className="historical-performance">
                            <h3>Historical Performance</h3>
                            <div className="performance-metrics">
                                <div className="metric"><h4>Average Completion Time</h4><p>{developerPerformance.average_completion_time?.toFixed(1) || 'N/A'} hours</p></div>
                                <div className="metric"><h4>Accuracy</h4><p>{(developerPerformance.accuracy * 100)?.toFixed(1) || 'N/A'}%</p></div>
                                <div className="metric"><h4>Total Completed Tickets</h4><p>{developerPerformance.total_completed_tickets || 'N/A'}</p></div>
                                <div className="metric"><h4>Average Sentiment</h4><p>{(developerPerformance.average_sentiment * 100)?.toFixed(1) || 'N/A'}%</p></div>
                            </div>
                        </div>
                    ) : (
                        <div className="no-performance-data"><p>No performance data available for this developer.</p></div>
                    )}
                    <div className="action-buttons">
                        <button className="btn btn-secondary" onClick={() => { setSelectedDeveloper(null); setDeveloperPerformance(null); }}>Back to Developers</button>
                        <button className="btn btn-primary" onClick={() => setActiveView('dashboard')}>Back to Dashboard</button>
                    </div>
                </div>
            )}
        ...
        <div className="process-document-view">
            ...
            <div className="document-form">
                <div className="form-group">
                    ...
                    <input type="text" id="documentPath" value={documentPath} onChange={(e) => setDocumentPath(e.target.value)} placeholder="Enter path to document (.docx, .txt)" required />
                </div>
                <div className="form-actions">
                    <button className="btn btn-primary" onClick={handleProcessDocument}>Process Document</button>
                    <button className="btn btn-secondary" onClick={() => setActiveView('dashboard')}>Cancel</button>
                </div>
            </div>
        ...
        <div className="process-sprint-document-view">
            ...
            <div className="document-form">
                <div className="form-group">
                    ...
                    <input type="text" id="sprintDocumentPath" value={sprintDocumentPath} onChange={(e) => setSprintDocumentPath(e.target.value)} placeholder="Example: C:\\Users\\shrad\\source\\repos\\smart_sprint_framework\\smart_sprint_framework\\sprint_documents\\SprintStory.docx" required />
                </div>
                <div className="form-actions">
                    <button className="btn btn-primary" onClick={handleProcessSprintDocument}>Process Sprint Document</button>
                    <button className="btn btn-secondary" onClick={() => setActiveView('dashboard')}>Cancel</button>
                </div>
            </div>
            <div className="document-info">
...
"As a user, I want to login with my credentials so that I can access my account" (Story Points: 3)`}
    ...
    const renderSaveDataView = () => (
        <div className="save-data-view">
            ...
            <div className="action-buttons">
                <button className="btn btn-primary" onClick={handleSaveData}>Save Data Now</button>
                <button className="btn btn-secondary" onClick={() => setActiveView('dashboard')}>Cancel</button>
            </div>
        </div>
    );
